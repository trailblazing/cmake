...
# variable_watch(languages)
# include (CheckLanguage)
#
# # https://stackoverflow.com/questions/32389273/detect-project-language-in-cmake
# # get_property(languages GLOBAL PROPERTY ENABLED_LANGUAGES)
# # message_format (STATUS "${lead_mark_location}" "compiler option" "languages"   "=" "${languages}")
# check_language(C)
# if(CMAKE_C_COMPILER)
#     get_property(languages GLOBAL PROPERTY ENABLED_LANGUAGES)
#     # if(C_NOTFOUND)
...
#         if(${_index} LESS 0)
#             if (NOT DEFINED _language_c_enabled)
#                 set (_language_c_enabled 1)
#                 # message_format (STATUS "${lead_mark_location}" "compiler option" "languages"   "=" "${languages}")
#                 enable_language(C)
#                 get_property(languages GLOBAL PROPERTY ENABLED_LANGUAGES)
#                 message_format (STATUS "${lead_mark_location}" "compiler option" "languages"   "=" "${languages}")
...
#         # if (NOT DEFINED _language_cxx_enabled)
#             set (_language_cxx_enabled 1)
#             enable_language(CXX)
#             get_property(languages GLOBAL PROPERTY ENABLED_LANGUAGES)
#             message_format (STATUS "${lead_mark_location}" "compiler option" "languages"   "=" "${languages}")
#         # endif()
...
# https://stackoverflow.com/questions/63268761/setting-d-variables-inside-cmake
...
# macro(compiler_options_setting DEPENDENCIES_INCLUDE_DIRS DEPENDENCIES_LIBRARY_DIRS DEPENDENCIES_LIBRARIES)
    ...
    message_format (STATUS "${lead_mark_location}" "compiler option" "clangplusplus_index" "=" "${clangplusplus_index}" )
    message_format (STATUS "${lead_mark_location}" "compiler option" "CMAKE_CXX_COMPILER_ID" "=" "${CMAKE_CXX_COMPILER_ID}" )
    # if ((NOT ${clangplusplus_index} EQUAL -1 ) AND (CMAKE_CXX_COMPILER_ID MATCHES "Clang"))
    if (NOT ${clangplusplus_index} EQUAL -1)
        ...
        if (${clangplusplus_index} EQUAL -1)
        ...
        list(LENGTH LLVM_DEFINITIONS_LIST definitons_length)
        if (${definitons_length} GREATER 0)
            string(STRIP ${LLVM_DEFINITIONS_LIST} LLVM_DEFINITIONS_LIST)
        endif ()
        add_definitions(${LLVM_DEFINITIONS_LIST})
        add_definitions(${LLVM_DEFINITIONS})

        message_format (STATUS "${lead_mark_location}" "compiler option" "LLVM_DEFINITIONS_LIST" "=" "${LLVM_DEFINITIONS_LIST}")
        message_format (STATUS "${lead_mark_location}" "compiler option" "LLVM_DEFINITIONS"      "=" "${LLVM_DEFINITIONS}")
        ...
        if ((DEFINED Clang) OR (Clang_FOUND))
            ...
            message_format (STATUS "${lead_mark_location}" "compiler option" "libClang_LIBRARY"        "=" "${libClang_LIBRARY}")
            message_format (STATUS "${lead_mark_location}" "compiler option" "libClang_INCLUDE_DIRS"   "=" "${libClang_INCLUDE_DIRS}")
            message_format (STATUS "${lead_mark_location}" "compiler option" "CLANG_DEFINITIONS"       "=" "${CLANG_DEFINITIONS}")
        endif ()
        ...
        string (FIND "${CMAKE_C_FLAGS}" "-Wall" initial_index_c)
        if (${initial_index_c} EQUAL -1)
            set (CMAKE_C_FLAGS                  "${CMAKE_C_FLAGS} -Wall")
        endif ()
        string (FIND "${CMAKE_C_FLAGS}" "-std=c99" initial_index_c)
        if (${initial_index_c} EQUAL -1)
            set (CMAKE_C_FLAGS                  "${CMAKE_C_FLAGS} -std=c99")
        endif ()
        string (FIND "${CMAKE_C_FLAGS}" "-flto" initial_index_c)
        if (${initial_index_c} EQUAL -1)
            set (CMAKE_C_FLAGS                  "${CMAKE_C_FLAGS} -flto")
        endif ()
        string (FIND "${CMAKE_C_FLAGS}" "-g" initial_index_c)
        if (${initial_index_c} EQUAL -1)
        ...
        string (FIND "${CMAKE_CXX_FLAGS}" "-Wall" initial_index_cxx)
        if (${initial_index_cxx} EQUAL -1)
        ...
        message_format (STATUS "${lead_mark_location}" "compiler option initial" "CMAKE_CXX_FLAGS_INIT" "=" "${CMAKE_CXX_FLAGS_INIT}")
        ...
        string (PREPEND CMAKE_CXX_FLAGS_INIT "")
        ...
        if (UNIX)
            if (BUILD_SHARED_LIBS)  # STATIC_LIBRARY, MODULE_LIBRARY, SHARED_LIBRARY, EXECUTABLE
                ...
                elseif (NOT ${clangplusplus_index} EQUAL -1)
        ...
        # add_compile_options (-stdlib=libc++ -v)     # add_compile_options(-stdlib=libc++)    # add_compile_options(-v)
        add_compile_options (-v)
        # set (CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -fno-exceptions")
        # add_compile_options (-fno-exceptions)       # add_compile_options(-stdlib=libc++)    # add_compile_options(-v)
        # add_compile_options(${CMAKE_C_FLAGS})       # add_compile_definitions(${CMAKE_C_FLAGS}) won't work
        # add_compile_options(${CMAKE_CXX_FLAGS})     # add_compile_definitions(${CMAKE_CXX_FLAGS})    # -D "-stdlib=libc++ -v"    # won't work


        # https://reviews.llvm.org/D49502
        string (PREPEND CMAKE_CXX_STANDARD_LIBRARIES "-lc++abi -lc++ -lpthread -lunwind ") # -static
        add_link_options(${CMAKE_CXX_STANDARD_LIBRARIES})
        ...
        message_format (STATUS "${lead_mark_location}" "compiler option" "llvm_libraries"            "=" "${llvm_libraries}")
        ...
        list (APPEND DEPENDENCIES_LIBRARIES "${llvm_libraries}")
...
# endmacro()
# compiler_options_setting(DEPENDENCIES_INCLUDE_DIRS DEPENDENCIES_LIBRARY_DIRS DEPENDENCIES_LIBRARIES)
...
# # https://stackoverflow.com/questions/7031126/switching-between-gcc-and-clang-llvm-using-cmake
# set (CMAKE_C_COMPILER               "${CLANG_INSTALL_PREFIX}/bin/clang")
# string (FIND "${CMAKE_C_FLAGS}" "-Wall -std=c99" initial_index_c)
# if (${initial_index_c} EQUAL -1)
